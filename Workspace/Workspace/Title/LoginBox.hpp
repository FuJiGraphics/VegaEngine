...
namespace fz {
	...
	class LoginButtonScript : public VegaScript
	{
	public:
		...
		void OnUpdate(float dt) override
		{
			if (IsClickedBounds())
			{
				if (Input::IsMouseButtonPressed(MouseButtonType::Left))
				{
					...
					GetComponent<SpriteComponent>().Sprite.SetTexture(
						"game/graphics/title/Login/loginObject/button/login/LoginPress.png");
				}
				else if (Input::IsMouseButtonReleased(MouseButtonType::Left))
				{
					if (isClickedLeft)
					{
						isClickedLeft = false;
						GetComponent<SpriteComponent>().Sprite.SetTexture(
							"game/graphics/title/Login/loginObject/button/login/LoginRelease.png");
						GameObject title = GetCurrentScene()->GetEntityFromTag("TitleFrame");
						title.GetComponent<FrameComponent>().status = FrameStatus::LoginDone;
					}
				}
			}
			else
			{
				GetComponent<SpriteComponent>().Sprite.SetTexture(
					"game/graphics/title/Login/loginObject/button/login/LoginHover.png");
			}
		}
